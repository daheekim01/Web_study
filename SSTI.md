...
* 이 페이로드는 **템플릿/표현식 주입(SSTI / Expression Injection) 탐지**용입니다.
* **직접적인 쉘 커맨드 인젝션** 문법은 아니지만, 템플릿 엔진이 `#{ ... }` 같은 표현을 평가하면 **서버 측에서 코드가 실행되거나 값이 반환**됩니다.
* SSTI가 가능하면 이후에 RCE(원격 코드 실행)로 발전할 가능성이 있어 심각하게 다뤄야 함.
...
* `<%\w7*777%>` (예시로 준 `<% ... %>` 계열)
  * `<% ... %>` 표기: **ERB (Ruby), EJS (Node), ASP-like** 템플릿에서 사용. 서버에서 렌더링되는 경우 내부 코드(언어 코드)를 실행할 가능성이 있음 — 매우 위험.
