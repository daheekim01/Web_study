...
## 3) 공격 기법 (Command Injection + PEAR 악용)

이 공격은 **명령어 삽입 (Command Injection)** 기법과 관련이 있습니다.
공격자는 `pearcmd`와 같은 PEAR 관련 명령어를 악용하여 **명령어를 실행**하고, 그 뒤에 **다른 악성 명령어나 스크립트**를 삽입하려 시도한 것입니다.

### 단계별 공격 흐름:

1. **디렉토리 탐색 (`../`)**: 공격자는 PEAR 명령어를 실행할 위치가 아닌 다른 디렉토리로 이동하려고 합니다. 이는 **경로 조작**을 통한 **경로 우회** 시도일 수 있습니다.
2. **PEAR 명령어 실행 (`pearcmd`)**: PEAR 명령어를 이용해 PHP 명령어 실행 권한을 얻으려 합니다.
3. **명령어 연결 (`&`)**: `&` 기호로 여러 명령어를 **한 줄에 연결**하여 추가적인 악성 명령어를 실행하려 시도합니다.
4. **악성 스크립트 (`/safedog()`)**: 시스템에 존재할 수 있는 악성 PHP 함수나 스크립트를 호출하여 **백도어**를 실행하거나 **시스템 침해**를 시도합니다.
5. **로그 파일 생성 (`bxf1GaJYWB.log`)**: 공격자는 결과를 로그 파일에 기록하여 **후속 작업**을 준비하거나 공격을 추적할 수 있습니다.
...
* 이 요청은 PEAR 명령어 실행 후, `&`로 명령어를 연결해 `/safedog()`라는 악성 함수 또는 스크립트를 실행하며, 결과를 `bxf1GaJYWB.log`라는 로그 파일에 저장하려는 의도가 있을 수 있습니다.
