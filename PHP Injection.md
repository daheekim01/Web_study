...
`eval-stdin.php` 의 동작: 요청(또는 stdin)으로 들어온 문자열을 PHP `eval()` 또는 등가 함수로 실행하도록 설계되어 있음(취약 버전에서). 따라서 전달된 `<?php ... ?>` 블록이 곧바로 PHP 인터프리터에서 실행됩니다.
그 결과 PHP 문법으로 작성된 코드가 서버 컨텍스트(웹서버 권한)에서 즉시 수행됩니다
