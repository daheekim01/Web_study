...
\*\*HTML 네임스페이스(Namespace)\*\*는 **문서에 어떤 종류의 태그(요소)가 포함되어 있는지 브라우저에게 알려주는 체계**입니다.

HTML, SVG, MathML 등은 **각기 다른 문법과 파싱 규칙**을 가지고 있기 때문에,
브라우저는 어떤 태그가 어떤 문법을 따라야 할지를 **"네임스페이스"를 보고 판단**합니다.
...
| 종류     | 설명                                      | 네임스페이스 URI                           |
| ------ | --------------------------------------- | ------------------------------------ |
| HTML   | 우리가 흔히 쓰는 `<div>`, `<p>` 등              | `http://www.w3.org/1999/xhtml`       |
| SVG    | `<svg>`, `<circle>`, `<style>` (SVG 전용) | `http://www.w3.org/2000/svg`         |
| MathML | 수학 표현 태그 `<math>`, `<mtext>` 등          | `http://www.w3.org/1998/Math/MathML` |

---

## 💡 왜 중요할까?

브라우저는 **네임스페이스에 따라 태그를 다르게 해석**합니다.

### 🔍 예시 1: `<style>` 태그의 파싱 차이

#### ✅ HTML 네임스페이스

```html
<style><a id="test"></style>
```

* **해석 결과**: `<a>`는 무시되고 **텍스트로 취급**됨.
* 이유: HTML에서 `<style>`의 내용은 "텍스트(content model: raw text)"로 간주됨.

#### ❗ SVG 네임스페이스

```html
<svg><style><a id="test"></style></svg>
```

* **해석 결과**: `<a>`가 **진짜 태그로 파싱**됨.
* 이유: SVG에서는 `<style>`이 자식을 가질 수 있으므로, 내부의 `<a>`가 DOM 요소로 등록됨 → mXSS 발생 가능!

---

### 🔍 예시 2: `<foreignObject>`로 HTML 네임스페이스 전환
...
* `<foreignObject>`는 SVG 내부에서 **HTML 네임스페이스를 다시 사용할 수 있게 해줌**.
* 브라우저는 `xmlns="http://www.w3.org/1999/xhtml"`을 보고 "아, 이건 HTML이구나!" 하고 판단.
* 이걸 악용해서 XSS를 시도하는 경우도 있음.
...
| 공격자 입장에서 중요한 이유                                                  |
| ---------------------------------------------------------------- |
| 브라우저는 태그의 네임스페이스에 따라 **완전히 다른 방식으로 파싱**함                         |
| sanitizer는 HTML 네임스페이스만 검사하고, SVG/MathML은 **제대로 검사하지 않는 경우가 많음** |
| 이걸 이용하면 sanitizer를 우회해서 **위험한 태그를 몰래 삽입(mutation)** 가능           |
