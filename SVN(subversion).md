## 🐻‍❄️ SVN (Subversion)
**SVN**은 **Subversion**의 약자로, **버전 관리 시스템(Version Control System, VCS)** 중 하나입니다. **파일과 디렉토리의 변경 이력을 추적**하고 소스 코드나 문서의 변경 이력을 중앙 서버에 저장하고 관리하는 중앙집중형 버전 관리 시스템으로, 주로 소프트웨어 개발에 사용됩니다.
...
1. **버전 관리 시스템 (VCS)**:
   ...
   * 모든 소스 코드와 변경 이력(로그)을 기록하고, 여러 사람이 **동시 작업**을 할 때 충돌을 방지하는 시스템입니다.
    
2. **중앙 집중식**:

   * SVN은 **중앙 서버**에 모든 데이터(파일, 폴더 등)와 변경 이력을 저장하는 **중앙 집중식 버전 관리 시스템**(Centralized Version Control System, CVCS)입니다.
...
| 기능                     | 설명                                       |
| ---------------------- | ---------------------------------------- |
| **저장소(Repository) 관리** | 프로젝트의 모든 파일과 디렉토리, 수정 이력(log), 메타데이터를 저장 |
| **버전 관리**              | 각 커밋마다 버전 번호를 부여하여 파일 변경 이력 추적 가능        |
| **로그 관리**              | 누가, 언제, 어떤 파일을, 어떻게 변경했는지 기록             |
| **접근 제어**              | 사용자별 권한(읽기/쓰기)을 설정하여 보안 관리               |
| **백업 및 복원**            | 저장소의 이력 전체를 백업 가능, 데이터 손실 대비             |
